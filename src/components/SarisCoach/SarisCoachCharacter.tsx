...
export default function SarisCoachCharacter({ gesture, isWalking = false, className = '' }: SarisCoachCharacterProps) {
  ...
  // Walking leg keyframes
  const leftLegWalk = isWalking
    ? { rotate: [15, -15, 15], x: 0, y: 0 }
    : { rotate: 0, x: 0, y: 0 };
  const rightLegWalk = isWalking
    ? { rotate: [-15, 15, -15], x: 0, y: 0 }
    : { rotate: 0, x: 0, y: 0 };
  ...
  const bodyBob = isWalking
    ? { y: [0, -2, 0, -2, 0] }
    : { y: [0, -1, 0] };
  const bodyTransition = isWalking
    ? { duration: 0.4, repeat: Infinity, ease: 'easeInOut' as const }
    : { duration: 2, repeat: Infinity, ease: 'easeInOut' as const, delay: 0.1 };

  // Arm swing while walking
  const leftArmWalk = isWalking
    ? { rotate: [-10, 10, -10], x: 0, y: 0 }
    : leftArmVariants[gesture];
  const rightArmWalk = isWalking
    ? { rotate: [10, -10, 10], x: 0, y: 0 }
    : rightArmVariants[gesture];
  const armTransition = isWalking
    ? { duration: 0.4, repeat: Infinity, ease: 'easeInOut' as const }
    : { duration: 0.6, ease: 'easeOut' as const };
  ...
  return (
    <div className={`relative select-none ${className}`} style={{ width: 100, height: 180 }}>
      <svg viewBox="0 0 100 180" width="100" height="180" fill="none" xmlns="http://www.w3.org/2000/svg">
        {/* Shadow under feet */}
        <motion.ellipse
          cx="50" cy="176" rx="22" ry="4"
          fill="hsl(215 20% 70% / 0.3)"
          animate={isWalking ? { rx: [22, 18, 22], opacity: [0.3, 0.2, 0.3] } : {}}
          transition={isWalking ? { duration: 0.4, repeat: Infinity } : {}}
        />

        {/* === LEFT LEG === */}
        <motion.g
          style={{ originX: '41px', originY: '130px' }}
          animate={leftLegWalk}
        ...
        {/* === RIGHT LEG === */}
        <motion.g
          style={{ originX: '59px', originY: '130px' }}
          animate={rightLegWalk}
          transition={legTransition}
        >
        ...
        >
          ...
          {!isWalking && (gesture === 'idle' || gesture === 'pointing-right') && (
            <g>
        ...
        {/* === HEAD === */}
        <motion.g
          animate={isWalking ? { y: [0, -1, 0, -1, 0] } : { y: [0, -1.5, 0] }}
